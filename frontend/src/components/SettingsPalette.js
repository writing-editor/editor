...
export class SettingsPalette {
  ...
  _onProviderChange() {
    ...
    apiKeyLabel.textContent = provider === 'google'
      ? 'Google Gemini API Key'
      : 'Anthropic API Key';

    // Show/hide the relevant model options
    this.containerEl.querySelectorAll('.provider-option').forEach(option => {
      option.hidden = !option.classList.contains(`${provider}-option`);
    });

    // Automatically select the first valid model for the new provider
      ...
      <div class="settings-section">
        <div class="setting-item">
          ...
          <select id="setting-provider">
            <option value="google" ${isGoogle ? 'selected' : ''}>Google Gemini</option>
            <option value="anthropic" ${!isGoogle ? 'selected' : ''}>Anthropic Claude</option>
          </select>
        </div>
        <div class="setting-item">
          <label id="setting-api-key-label" for="setting-api-key">
            ${isGoogle ? 'Google Gemini API Key' : 'Anthropic API Key'}
          </label>
        ...
        <div class="setting-item">
          ...
          <select id="setting-model-name">
            <!-- Google Models -->
            <option class="provider-option google-option" value="gemini-2.5-flash-lite" ${settings.modelName === 'gemini-2.5-flash-lite' ? 'selected' : ''}>
              Gemini 2.5 Flash-lite (R)
            </option>
            <option class="provider-option google-option" value="gemini-2.5-pro" ${settings.modelName === 'gemini-2.5-pro' ? 'selected' : ''}>
              Gemini 2.5 pro
            </option>
            <option class="provider-option google-option" value="gemini-2.5-flash" ${settings.modelName === 'gemini-2.5-flash' ? 'selected' : ''}>
              Gemini 2.5 Flash
            </option>
            <!-- Anthropic Models -->
            <option class="provider-option anthropic-option" value="claude-3-haiku-20240307" ${settings.modelName === 'claude-3-haiku-20240307' ? 'selected' : ''}>
              Claude 3 Haiku (Fast)
            </option>
            <option class="provider-option anthropic-option" value="claude-3-sonnet-20240229" ${settings.modelName === 'claude-3-sonnet-20240229' ? 'selected' : ''}>
              Claude 3 Sonnet
            </option>
          </select>
        </div>
    ...
    return `
      <div class="settings-section">
        <!-- Item 1: Local Backup -->
          ...
          <!-- Item 4: Disconnect and Reset -->
          <div class="setting-item-full">
            <label class="item-label">Disconnect & Reset</label>
            <p class="item-description">
              This will unregister the service worker, delete all local data (documents, notes, and settings) from your
              browser, and reload the application.
              This is the equivalent of a full uninstall.
            </p>
            <div class="data-actions">

              <button id="settings-disconnect-btn" class="settings-action-btn"
                title="Disconnect and Clear All Local Data">
